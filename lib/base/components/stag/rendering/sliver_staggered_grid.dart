...
import 'package:flutter_starter/base/components/stag/rendering/sliver_variable_size_box_adaptor.dart';

import '../widgets/staggered_tile.dart';


/// Signature for a function that creates [CStaggeredTile] for a given index.
typedef IndexedStaggeredTileBuilder = CStaggeredTile? Function(int index);
...
class StaggeredGridConfiguration {
  ...
  CStaggeredTile? getStaggeredTile(int index) {
    CStaggeredTile? tile;
  ...
  double _getStaggeredTileMainAxisExtent(CStaggeredTile tile) {
  ...
  CStaggeredTile? _normalizeStaggeredTile(CStaggeredTile? staggeredTile) {
    if (staggeredTile == null) {
      ...
      if (staggeredTile.fitContent) {
        return CStaggeredTile.fit(crossAxisCellCount);
      } else {
        return CStaggeredTile.extent(
